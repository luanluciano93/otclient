...
function charmsControllerCyclopedia:onInit()
	...
	charmsControllerCyclopedia:registerEvents(g_game, {
		onParseCharm = Cyclopedia.onParseCharm
	})
end
...
local CHARMS = {
	{
		ID = 9,
		IMAGE = "/mods/game_cyclopedia/images/charms/adrenaline_burst",
		TYPE = 1
	},
	{
		ID = 11,
		IMAGE = "/mods/game_cyclopedia/images/charms/cleanse",
		TYPE = 1
	},
	{
		ID = 10,
		IMAGE = "/mods/game_cyclopedia/images/charms/numb",
		TYPE = 1
	},
	{
		ID = 6,
		IMAGE = "/mods/game_cyclopedia/images/charms/cripple",
		TYPE = 2
	},
	{
		ID = 8,
		IMAGE = "/mods/game_cyclopedia/images/charms/dodge",
		TYPE = 3
	},
	{
		ID = 7,
		IMAGE = "/mods/game_cyclopedia/images/charms/parry",
		TYPE = 3
	},
	{
		ID = 5,
		IMAGE = "/mods/game_cyclopedia/images/charms/curse",
		TYPE = 4
	},
	{
		ID = 1,
		IMAGE = "/mods/game_cyclopedia/images/charms/enflame",
		TYPE = 4
	},
	{
		ID = 3,
		IMAGE = "/mods/game_cyclopedia/images/charms/freeze",
		TYPE = 4
	},
	{
		ID = 2,
		IMAGE = "/mods/game_cyclopedia/images/charms/poison",
		TYPE = 4
	},
	{
		ID = 0,
		IMAGE = "/mods/game_cyclopedia/images/charms/wound",
		TYPE = 4
	},
	{
		ID = 4,
		IMAGE = "/mods/game_cyclopedia/images/charms/zap",
		TYPE = 4
	},
	{
		ID = 16,
		IMAGE = "/mods/game_cyclopedia/images/charms/divine_wrath",
		TYPE = 5
	},
	{
		ID = 15,
		IMAGE = "/mods/game_cyclopedia/images/charms/low_blow",
		TYPE = 6
	},
	{
		ID = 17,
		IMAGE = "/mods/game_cyclopedia/images/charms/vampiric_embrace",
		TYPE = 6
	},
	{
		ID = 18,
		IMAGE = "/mods/game_cyclopedia/images/charms/voids_call",
		TYPE = 6
	}
}
...
function Cyclopedia.CreateCharmItem(data)
	...
	widget.charmBase.image:setImageSource(CHARMS[data.internalId].IMAGE)
	...
	if data.unlocked then
		...
		if data.hasCurrentCreature then
			widget.PriceBase.Value:setText(comma_value(data.charmPrice))
		else
		...
		widget.PriceBase.Value:setText(comma_value(data.price))

		widget.icon = 0
	end

	if widget.icon == 1 and UI.Balance then
		if data.charmPrice > UI.Balance then
	...
	if widget.icon == 0 then
		if data.price > UI.CharmsPoints then
			value:setColor("#D33C3C")
		else
			value:setColor("#C0C0C0")
		end
...
function Cyclopedia.loadCharms(points, data, monsters)
	...
	for _, charmData in pairs(data) do
		local internalId = verify(charmData.id)

		if internalId then
			charmData.internalId = internalId
			charmData.typePriority = CHARMS[internalId].TYPE
	...
	if Cyclopedia.Charms.redirect then
		Cyclopedia.selectCharm(UI.CharmList:getChildById(Cyclopedia.Charms.redirect), UI.CharmList:getChildById(Cyclopedia.Charms.redirect):isChecked())
...
function Cyclopedia.selectCharm(widget, isChecked)
	...
	if widget.icon == 1 and UI.Balance then
		if widget.data.charmPrice > UI.Balance then
			value:setColor("#D33C3C")
		...
		if widget.data.unlocked and not widget.data.hasCurrentCreature then
			...
			value:setText(comma_value(widget.data.charmPrice))
		end
	end

	if widget.icon == 0 then
		if widget.data.price > UI.CharmsPoints then
			value:setColor("#D33C3C")
			button:setEnabled(false)
		else
			value:setColor("#C0C0C0")
		...
		value:setText(widget.data.price)
	end
...
function Cyclopedia.actionCharmButton(widget)
	...
	if type == "Unlock" then
		...
		if not confirmWindow then
			confirmWindow = displayGeneralBox(tr("Confirm Unlocking of Charm"), tr("Do you want to unlock the Charm %s? This will cost you %d Charm Points?", data.name, data.price), {
				{
					text = tr("Yes"),
	...
	if type == "Remove" then
		...
		if not confirmWindow then
			confirmWindow = displayGeneralBox(tr("Confirm Charm Removal"), tr("Do you want to remove the Charm %s from this creature? This will cost you %s gold pieces.", data.name, comma_value(data.charmPrice)), {
				{
					text = tr("Yes"),
					callback = yesCallback
				},
