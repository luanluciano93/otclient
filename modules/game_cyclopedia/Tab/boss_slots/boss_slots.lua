...
function boss_slotControllerCyclopedia:onInit()
    boss_slotControllerCyclopedia:registerEvents(g_game, {
		onParseBosstiarySlot = Cyclopedia.onParseBosstiarySlot,
	})
...
function Cyclopedia.BossSlotsLoad(data, slots, unlockedBosses)
	Cyclopedia.BossSlots.UnlockBosses = {}

	UI.Sprite:setOutfit(RACE_TEST[data.boostedBossId].outfit)
	UI.Sprite:setAnimate(true)

	local fullText = ""

	if data.boostedKills >= CONFIG[data.boostedCategory].MASTERY then
		fullText = "(fully unlocked)"
	end

	local progress = UI.BoostedProgress

	progress.ProgressBorder1:setTooltip(string.format(" %d / %d %s", data.boostedKills, CONFIG[data.boostedCategory].PROWESS, fullText))
	progress.ProgressBorder2:setTooltip(string.format(" %d / %d %s", data.boostedKills, CONFIG[data.boostedCategory].EXPERTISE, fullText))
	progress.ProgressBorder3:setTooltip(string.format(" %d / %d %s", data.boostedKills, CONFIG[data.boostedCategory].MASTERY, fullText))

	if data.boostedKills >= CONFIG[data.boostedCategory].PROWESS then
	...
	if data.boostedKills >= CONFIG[data.boostedCategory].EXPERTISE then
	...
	for i = 0, #unlockedBosses do
		local unlockData = unlockedBosses[i]

		if not unlockData then
			return
		end

		local raceData = RACE_TEST[unlockData.bossId]
	...
	UI.TopBase.InfoLabel:setText(string.format("Equipment Loot Bonus: %d%% Next: %d%%", data.bossLootBonus, data.bossLootBonus + 1))
	UI.MainLabel:setText(string.format("Equipment loot bonus: %d%%\nKill bonus: %dx", data.boostedBonusValue, data.boostedKillValue))
	...
	UI.MidTitle:setText(string.format("Boosted Boss: %s", format(RACE_TEST[data.boostedBossId].name)))
	...
	tooltip = data.boostedCategory == CATEGORY.ARCHFOE and "Archfoe\n\nFor unlocking a level, you will receive the following boss points:\nProwess: 10\nExpertise: 30\nMastery: 60" or "Nemesis\n\nFor unlocking a level, you will receive the following boss points:\nProwess: 10\nExpertise: 30\nMastery: 60"

	UI.TypeIcon:setTooltip(tooltip)
	--UI.TypeIcon:setTooltipAlign(AlignTopLeft)

	for slotId, slotData in ipairs(slots) do
...
function Cyclopedia.BossSlotChangeSlot(slot, data, unlockedBosses)
	...
	if data.state == SLOT_STATE.LOCKED then
		...
		widget.LockLabel:setText(string.format("Unlocks at %d Boss Points", data.slotUnlockedPoints))
	elseif data.state == SLOT_STATE.EMPTY then
		...
		for _, internalData in ipairs(Cyclopedia.BossSlots.UnlockBosses) do
			...
			internalWidget.Sprite:setOutfit(RACE_TEST[internalData.bossId].outfit)
			internalWidget:setText(format(RACE_TEST[internalData.bossId].name))
	...
	elseif data.state == SLOT_STATE.ACTIVE then
		...
		widget:setText(string.format("Slot %d: %s", slot, RACE_TEST[data.bossId].name))
		...
		tooltip = data.category == CATEGORY.ARCHFOE and "Archfoe\n\nFor unlocking a level, you will receive the following boss points:\nProwess: 10\nExpertise: 30\nMastery: 60" or "Nemesis\n\nFor unlocking a level, you will receive the following boss points:\nProwess: 10\nExpertise: 30\nMastery: 60"
		...
		progress.ProgressBorder1:setTooltip(string.format(" %d / %d %s", data.kills, CONFIG[data.category].PROWESS, fullText))
		progress.ProgressBorder2:setTooltip(string.format(" %d / %d %s", data.kills, CONFIG[data.category].EXPERTISE, fullText))
		progress.ProgressBorder3:setTooltip(string.format(" %d / %d %s", data.kills, CONFIG[data.category].MASTERY, fullText))
		...
		widget.ActivedBoss.Sprite:setOutfit(RACE_TEST[data.bossId].outfit)
		...
		widget.ActivedBoss.RemoveButton:setTooltip(string.format("It will cost you %s gold to remove the currently selected boss from this slot.", comma_value(data.goldToRemove)))
	end
end
