...
function Cyclopedia.rejectTransfer()
    ...
    UI.rejectTransferHouse.rent:setText(comma_gold(house.rent))
...
function Cyclopedia.acceptTransfer()
    ...
    UI.acceptTransferHouse.rent:setText(comma_gold(house.rent))
...
function Cyclopedia.cancelTransfer()
    ...
    UI.cancelHouseTransferArea.rent:setText(comma_gold(house.rent))
...
function Cyclopedia.transferHouse()
    ...
    UI.transferArea.rent:setText(comma_gold(house.rent))
...
function Cyclopedia.moveOutHouse()
    ...
    UI.moveOutArea.rent:setText(comma_gold(house.rent))
...
function Cyclopedia.bidHouse(widget)
    ...
    UI.bidArea.rent:setText(comma_gold(house.rent))
    ...
    if house.hasBid then
        for index, data in ipairs(labels) do
            ...
            if data.id == "highestBid" then
                ...
                gold:setImageSource("/mods/game_cyclopedia/images/icon_gold")
    ...
    if house.hasBid then
        bidArea.information:setText(string.format(
            ...
            time, comma_gold(house.rent)))
    else
        bidArea.information:setText("When the auction ends, the winning bid plus the rent for\nthe first month( " ..
                                        comma_gold(house.rent) .. ") will de debited yo your bank account.")
    end
    ...
    function UI.bidArea.bid:onClick()
        ...
        if not confirmWindow then
            confirmWindow = displayGeneralBox(tr("Confirm House Action"), tr(
                ...
                house.name, comma_value(value), comma_gold(house.rent)), {
                {
                    text = tr("Yes"),
                    callback = yesCallback
                },
    ...
    if not table.empty(Cyclopedia.House.Data) then
        ...
        for _, data in ipairs(Cyclopedia.House.Data) do
            if data.visible then
                ...
                widget.size:setColoredText({"Size:     ", "#909090", data.sqm .. " sqm", "#C0C0C0"})
                widget.beds:setColoredText({"Max. Beds:  ", "#909090", data.beds, "#C0C0C0"})
                widget.rent:setColoredText({"Rent:  ", "#909090", comma_gold(data.rent), "#C0C0C0"})

                if data.description ~= "" then
                    local icon = g_ui.createWidget("HouseIcon", widget.icons)

                    icon:setImageSource("/mods/game_cyclopedia/images/house-description")
                ...
                if data.state == 0 then
                    if data.hasBid then
                        ...
                        widget.status:setColoredText({"Status:  ", "#909090", "auctioned", "#00F000",
                                                      " (Bid: " .. data.hightestBid .. " Ends in: " ..
                            format(data.bidEnd) .. ")", "#C0C0C0"})
                    else
                        widget.status:setColoredText({"Status:  ", "#909090", "auctioned", "#00F000", " (no bid yet)",
                                                      "#C0C0C0"})
                    end
                elseif data.state == 2 then
                    widget.status:setColoredText({"Status:  ", "#909090", "rented by " .. data.owner, "#C0C0C0"})
                end
                ...
                if data.isYourOwner then
                    ...
                    icon:setImageSource("/mods/game_cyclopedia/images/house-owner-icon")
                end

                if widget.data.isTransferOwner then
                    local icon = g_ui.createWidget("HouseIcon", widget.icons)

                    icon:setImageSource("/mods/game_cyclopedia/images/pending-transfer-house")
                end

                if data.isYourOwner and data.inTransfer then
                    local icon = g_ui.createWidget("HouseIcon", widget.icons)

                    icon:setImageSource("/mods/game_cyclopedia/images/transfer-house")
                end

                if data.shop then
                    local icon = g_ui.createWidget("HouseIcon", widget.icons)

                    icon:setImageSource("/mods/game_cyclopedia/images/house-shop")
...
function Cyclopedia.selectHouse(widget)
    ...
    if widget.data.hasBid then
        ...
        UI.LateralBase.AuctionText:setColoredText({"Hightest Bidder: ", "#909090", widget.data.bidName, "#C0C0C0",
                                                   "\n      End Time: ", "#909090", date, "#C0C0C0",
                                                   "\n   Highest Bid: ", "#909090"})
    ...
    elseif widget.data.rented or widget.data.inTransfer then
        ...
        UI.LateralBase.AuctionText:setColoredText({"            Tenant: ", "#909090", widget.data.owner, "#C0C0C0",
                                                   "\n         Paid Until: ", "#909090", date, "#C0C0C0"})
    ...
    local imagePath = string.format("/mods/game_cyclopedia/images/houses/%s.png", widget.data.id)
