...
function Cyclopedia.CreateBosstiaryCreature(data)
	...
	local raceData = RACE_TEST[data.raceId]
	local icons = {
		[CATEGORY.BANE] = "/mods/game_cyclopedia/images/boss/icon_bane",
		[CATEGORY.ARCHFOE] = "/mods/game_cyclopedia/images/boss/icon_archfoe",
		[CATEGORY.NEMESIS] = "/mods/game_cyclopedia/images/boss/icon_nemesis"
	}
	...
	if data.kills >= CONFIG[data.category].PROWESS then
		widget.bronzeStar:setImageSource("/mods/game_cyclopedia/images/boss/icon_star_bronze")
	else
		widget.silverStar:setImageSource("/mods/game_cyclopedia/images/boss/icon_star_dark")
	end

	if data.kills >= CONFIG[data.category].EXPERTISE then
		widget.silverStar:setImageSource("/mods/game_cyclopedia/images/boss/icon_star_silver")
	else
		widget.silverStar:setImageSource("/mods/game_cyclopedia/images/boss/icon_star_dark")
	end

	if data.kills >= CONFIG[data.category].MASTERY then
		widget.goldStar:setImageSource("/mods/game_cyclopedia/images/boss/icon_star_gold")
	else
		widget.goldStar:setImageSource("/mods/game_cyclopedia/images/boss/icon_star_dark")
	end
...
function Cyclopedia.LoadBoostiaryCreatures(data)
	...
	for i = 0, #data do
		local raceData = RACE_TEST[data[i].raceId]
		local creature = {
			visible = true,
			...
			isTrackerActived = data[i].isTrackerActived,
			unlocked = data[i].kills > 0 and true or false
		}
